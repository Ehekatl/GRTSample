...
namespace GRT {
/**
   @brief The CircularBuffer class provides a data structure for creating a
      dynamic circular buffer (also known as a
   cyclic buffer or a ring buffer).  The main advantage of a circular buffer is
      that it does not need to have
   its elements shuffled around each time a new element is added.  The circular
      buffer therefore works well
...
public:
  ...
  /**
     Copy Constructor. Deep copies the data from the rhs instance to this
        instance

     @param const CircularBuffer &rhs: rhs instance the data and settings will
        be copied from
   */
  CircularBuffer(const CircularBuffer& rhs) {
    ...
    if (rhs.bufferInit) {
      ...
      for (uint32 i = 0; i < rhs.bufferSize; i++) {
  ...
  /**
     ...
     @param uint32 bufferSize: sets the size of the buffer
   */
  CircularBuffer(uint32 bufferSize) {
    bufferInit = false;
    resize(bufferSize);
  }
  ...
  /**
     Sets the equals operator. Defines how the data will be copied from the rhs
  ...
  /**
     This is the main access operator and will return a value relative to the
        current read pointer.

     @param const uint32 &index: the index of the element you want access to,
        should be in the range [0 bufferSize-1]
  ...
  /**
     This is the constant access operator and will return a value relative to
  ...
  /**
     This is a special access operator that will return the value at the
        specific index (regardless of the position of the read pointer).
  ...
  /**
     This is a special const access operator that will return the value at the
        specific index (regardless of the position of the read pointer).
  ...
  /**
     ...
     @param const uint32 newBufferSize: the new size of the buffer
     @return returns true if the buffer was resized
   */
  bool resize(const uint32 newBufferSize) {
    return resize(newBufferSize, T());
  }

  /**
     Resizes the buffer to the newBufferSize, which must be greater than zero,
        and sets all the values to the default value.

     @param const uint32 newBufferSize: the new size of the buffer
     @param const T &defaultValue: the default value that will be copied to
        every element
     @return returns true if the buffer was resized
   */
  bool resize(const uint32 newBufferSize, const T& defaultValue) {
  ...
  /**
     Push the new value into the end of the buffer, this will move both the read
        and write pointers.

     @param const T &value: the value that should be added to the end of the
        buffer
  ...
  bool push_back(const T& value) {
    if (!bufferInit) {
      UE_LOG(GRTModule, Error,
             TEXT(
               "Can't push_back value to circular buffer as the buffer has not been initialized!"));
      return false;
    }
  ...
  /**
     ...
     @param const T &value: the value that will be copied to all the elements in
        the buffer
  ...
  /**
     Gets the data in the CircularBuffer's internal buffer as a std::vector.
         The argument, rawBuffer, controls if the function
     returns (if rawBuffer = true ) the entire contents of the internal buffer
        (with unsorted values and a constant size regardless on whether it is
        filled),
     or the current contents of the Circular Buffer in order from oldest to
        newest (if rawBuffer = false).
  ...
  Vector<T>getData(const bool rawBuffer = false) const {
    if (bufferInit) {
      ...
      else {           // Here we return only valid elements and in order from
                       // oldest to newest
        Vector<T> data(numValuesInBuffer);

        for (uint32 i = 0; i < numValuesInBuffer; i++) {
  ...
  /**
     Returns true if the buffer has been filled. If the buffer has not been
        initialized then this function will always return false.

     @return returns true if the buffer has been filled, false otherwise
   */
  bool getBufferFilled() const {
    return bufferInit ? numValuesInBuffer == bufferSize : false;
  }
   ...
   */
  uint32 getNumValuesInBuffer() const {
   ...
   */
  uint32 getReadPointerPosition() const {
   ...
   */
  uint32 getWritePointerPosition() const {
