...
namespace GRT {
  ...
  public:
    ...
    */
    virtual bool resize(const uint32 r, const uint32 c) {
      ...
      if (r + c == 0) {
        UE_LOG(GRTModule, Error, TEXT("Failed to resize matrix, rows and cols == zero!"));
        return false;
      }
      ...
      if (r > 0 && c > 0) {
        ...
        if (dataPtr == NULL) {
          ...
          size = 0;
          capacity = 0;
          UE_LOG(GRTModule, Error, TEXT("resize(const uint32 r,const uint32 c) - Failed to allocate memory! r: %f, c: %f"), r, c);
          clear();
          return false;
        }
      ...
      if (this != &rhs) {
        ...
        if (this->size != rhs.size) {
          if (!this->resize(rhs.rows, rhs.cols)) {
            UE_LOG(GRTModule, Error, TEXT("Matrix::copy( const Matrix<T> &rhs ) - Failed to allocate resize matrix!"));
            return false;
          }
        }
    ...
    */
    inline uint32 getNumRows() const { return rows; }
    ...
    */
    inline uint32 getNumCols() const { return cols; }
    ...
    */
    T** getDataPointer() const {
      if (rowPtr == NULL) {
        UE_LOG(GRTModule, Error, TEXT("Matrix::getDataPointer() - Matrix has not been initialized!"));
        return NULL;
      }
      return &(rowPtr[0]);
    }
  ...
  protected:
    ...
    uint32 capacity;  ///< The capacity of the Matrix, this will be the number of rows, not the actual memory size
