...
bool MatrixFloat::scale(const Vector<MinMax>& ranges,
  ...
  for (i = 0; i < rows; i++) {
    for (j = 0; j < cols; j++) {
      dataPtr[i * cols + j] = grt_scale(dataPtr[i * cols + j],
                                        ranges[j].minValue,
                                        ranges[j].maxValue,
                                        minTarget,
                                        maxTarget);
...
VectorFloat MatrixFloat::multiple(const VectorFloat& b) const {
  ...
  if (N != K) {
    UE_LOG(GRTModule, Warning,
           TEXT(
             "multiple(vector b) - The size of b %d does not match the number of columns in this matrix %d"), K,
           N);
    return VectorFloat();
  }
...
bool MatrixFloat::multiple(const MatrixFloat& a,
  ...
  if (N != K) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "multiple(const MatrixFloat &a,const MatrixFloat &b,const bool aTranspose) - The number of rows in a %d does not match the number of columns in matrix b %d"), K,
           N);
    return false;
  }
  ...
  if (!resize(M, L)) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "multiple(const MatrixFloat &b,const MatrixFloat &c,const bool bTranspose) - Failed to resize matrix!"), K,
           N);
    return false;
  }
...
bool MatrixFloat::add(const MatrixFloat& b) {
  if (b.getNumRows() != rows) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "add(const MatrixFloat &b) - Failed to add matrix! The rows do not match!"));
    return false;
  }
  ...
  if (b.getNumCols() != cols) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "add(const MatrixFloat &b) - Failed to add matrix! The cols do not match!"));
    return false;
  }
...
bool MatrixFloat::add(const MatrixFloat& a, const MatrixFloat& b) {
  ...
  if (M != b.getNumRows()) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "add(const MatrixFloat &a,const MatrixFloat &b) - Failed to add matrix! The rows do not match!, a rows: %d b rows: %d"), M,
           (uint32)b.getNumRows());
    return false;
  }
  ...
  if (N != b.getNumCols()) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "add(const MatrixFloat &a,const MatrixFloat &b) - Failed to add matrix! The columns do not match!, a cols: %d b cols: %d"), N,
           (uint32)b.getNumCols());
    return false;
  }
...
bool MatrixFloat::subtract(const MatrixFloat& b) {
  if (b.getNumRows() != rows) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "subtract(const MatrixFloat &b) - Failed to add matrix! The rows do not match! b rows: %d"),
           (uint32)b.getNumRows());
    return false;
  }
  ...
  if (b.getNumCols() != cols) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "subtract(const MatrixFloat &b) - Failed to add matrix! The cols do not match! b cols: %d"),
           (uint32)b.getNumCols());
    return false;
  }
...
bool MatrixFloat::subtract(const MatrixFloat& a, const MatrixFloat& b) {
  ...
  if (M != b.getNumRows()) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "subtract(const MatrixFloat &a,const MatrixFloat &b) - Failed to add matrix! The rows do not match! a rows: %d b rows: %d"), M,
           (uint32)b.getNumRows());
    return false;
  }
  ...
  if (N != b.getNumCols()) {
    UE_LOG(GRTModule, Error,
           TEXT(
             "subtract(const MatrixFloat &a,const MatrixFloat &b) - Failed to add matrix! The cols do not match! a cols: %d b cols: %d"), N,
           (uint32)b.getNumCols());
    return false;
  }
...
VectorFloat MatrixFloat::getStdDev() const {
  ...
  for (uint32 j = 0; j < cols; j++) {
    for (uint32 i = 0; i < rows; i++) {
      stdDev[j] += (dataPtr[i * cols + j] - mean[j]) *
...
MatrixFloat MatrixFloat::getCovarianceMatrix() const {
  ...
  for (uint32 j = 0; j < cols; j++) {
    for (uint32 k = 0; k < cols; k++) {
      ...
      for (uint32 i = 0; i < rows; i++) {
        covMatrix[j][k] += (dataPtr[i * cols + j] - mean[j]) *
